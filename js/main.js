...
$(document).ready(function($) {
		...
		if(windowWidth > 767 ){
			$(window).on('scroll', function(){
				if( $(window).scrollTop()>767 ){
					...
					$('.subnav').addClass('subnav-fixed-top animated fadeIn');
					$('.navbar').removeClass('main-nav');
					$(".breadcrumb-block.corporate").css("top","129px");
				} else {
					$('.navbar').removeClass('navbar-fixed-top');
					$('.subnav').removeClass('subnav-fixed-top');
					$('.navbar').addClass('main-nav');
					$(".breadcrumb-block.corporate").css("top","179px");
				}
			});
		}else{
	...
	function Scroll() {
		...
		$('.brand-promotion').find('span.select_btn').each(function(){
			contentTop.push( $( $(this).attr('href') ).offset().top);
			contentBottom.push( $( $(this).attr('href') ).offset().top + $( $(this).attr('href') ).height() );
		})
		$('#navigation').find('.book_com').each(function(){
			contentTop.push( $( $(this).attr('href') ).offset().top);
			contentBottom.push( $( $(this).attr('href') ).offset().top + $( $(this).attr('href') ).height() );
		})
